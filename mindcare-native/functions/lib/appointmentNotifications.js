...
exports.onAppointmentStatusChange = void 0;
...
exports.onAppointmentStatusChange = functions
    ...
    // Extract key fields
    const { patientId, doctorId, date, timeSlot, status: newStatus, } = afterData;
    ...
    console.log(`Appointment ${appointmentId}: ${oldStatus || 'NEW'} → ${newStatus}`);
    ...
    try {
        ...
        console.log(`✓ Notification queued for ${recipientType} (${recipientId}): "${messageTitle}"`);
    }
    catch (error) {
...
async function dispatchNotification(payload) {
    const { recipientId, recipientType, recipientName, messageTitle, messageBody, appointmentId, appointmentData, } = payload;
...
//# sourceMappingURL=appointmentNotifications.js.map